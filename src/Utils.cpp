...
namespace BrickArea {
	...
	const int Brick::WIDTH = Display::WIDTH / BrickArea::COLUMN_COUNT;
	const int Brick::HEIGHT = (Display::HEIGHT / 2) / BrickArea::ROW_COUNT;
	const int Brick::COLOR_COUNT = 9;
...
void MovableSprite::Init(int min, int max) {
	x = y = 0;
...
void MovableSprite::AdjustIfOutOfBounds() {
	if (x < 0 || x + width > Display::WIDTH)
		xDir = -xDir;
	else if (y < 0 || y + height > Display::HEIGHT)
		yDir = -yDir;
}
bool MovableSprite::TryMove() {
	f_xDir = xDir;
	f_yDir = yDir;
	AdjustIfOutOfBounds();
	OffsetAngleRandomly();
...
void MovableSprite::OffsetAngleRandomly() {
	if (xDir != f_xDir)
